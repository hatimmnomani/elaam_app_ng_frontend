...
@Component({
  selector: 'app-qardan-hasanah',
  templateUrl: './qardan-hasanah.component.html',
  styleUrls: ['./qardan-hasanah.component.scss']
})
export class QardanHasanahComponent implements OnInit {
  ...
  getListCall(text:string){
        this.fmbService.postActiveChangeList(this.dateDetails, this.labelTab === 'Niyat' ? "niyatQuestion" : this.labelTab , this.departmentId, 'getNiyatListV2',this.niyatStatus,text,this.jamiatId)
  ...
  getDataList(label: string,event: any,departmentId: number, status: number, search: string, id:any): void {
    if(label) {
      this.fmbService.getListData('getDepartmentActiveJamiatNiyatList', event,departmentId,status, search, id)
      .pipe(takeUntil(this.destroy$))
      .subscribe((data) => {
   ...
   ******************************************************************************/
    ...
    getNiyatStatus(event: any): void {
      this.fmbService.getNiyatStatus(event,'getDepartmentNiyatStatusData',this.departmentId)
      .pipe(takeUntil(this.destroy$))
      .subscribe((data) => {
  ...
  getChartData(label: string, event: any, status: number, id:any): void {
    this.getChartData$ = [];
    this.fmbService.getDataChart('getDepartmentDataInMap', event, status, this.departmentId,this.id)
    .pipe(takeUntil(this.destroy$))
    .subscribe((data) => {
