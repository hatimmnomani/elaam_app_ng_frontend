...
import { Component, OnInit, Output, ViewChild, Input, EventEmitter, SimpleChanges, ChangeDetectorRef } from '@angular/core';
...
@Component({
  selector: 'app-report-side-smart-table',
  templateUrl: './report-side-smart-table.component.html',
  styleUrls: ['./report-side-smart-table.component.scss']
})

export class ReportSideSmartTableComponent implements OnInit {
  ...
  @Input() set searchStr(value: string) {
    ...
    // Avoid triggering load on tab-change when value becomes empty
    // if (!value && this.isRestrictLoad) {
    //   return;
    // }
...
isIndeterminate(): boolean {
  ...
  loadData() {
    ...
    if (this.isApproved) {
      ...
      Object.assign(params, { itsId: itsID, isApproved: this.isApproved });
    }

    if (this.niyatList && this.niyatListVal?.scanned === false) {
      Object.assign(params, {
        download: this.niyatListVal?.scanned,
        endDate: this.niyatListVal?.endDate,
        startDate: this.niyatListVal?.startDate,
        templateId: this.niyatListVal?.templateId
      });
    } else {
      if (this.query) {
        Object.assign(params, { searchValue: this.query, search: this.query });
      } else {
        Object.assign(params, { searchValue: "", search: "" });
      }

      if (this.catelogType != undefined) {
        Object.assign(params, { catalogueType: this.catelogType, itemId: this.itemId, itsId: this.query });
      }

      if (this._filters) {
        const tempFilters = { ...this._filters };
        Object.assign(params, tempFilters);
      }

      if (this.niyatList && this.niyatListVal?.scanned === true) {
        Object.assign(params, { scanned: this.niyatListVal?.scanned, itsId: this.niyatListVal?.itsId });
      }
    }
    ...
    this.service.getList(params).subscribe((data: any) => {
      ...
      if (data.list && data.list.length > 0) {
        this.listSpinner = true;
        this.receivedData = this.isApproved ? this.isApprovalList(data.list) : data.list;
        this.receivedData.forEach((row: any, index: number) => {
          ...
          if (this.selectedRowMap.has(row.uniqueId)) {
            this.selection.select(row);
          }
        });
        this.dataSource = new MatTableDataSource(this.receivedData);
